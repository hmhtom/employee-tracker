...
const getAllEmployeeQuery = `SELECT 
employee.id, 
...
const getAllRoleQuery = `SELECT id,title AS Title,salary as Salary FROM role`;

//Get all department from department table
const getAllDepartmentQuery = `SELECT id, name AS Department FROM department`;
...
const updateEmployeeQuery = `UPDATE employee SET role_id = (SELECT id FROM role WHERE title = ?) WHERE concat(first_name," ",last_name) = ?`;
